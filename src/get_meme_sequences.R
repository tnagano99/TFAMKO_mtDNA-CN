...
flanking <- flank(ranges, 500, start = TRUE)

seqlevelsStyle(flanking) <- "UCSC"

seqs <- getSeq(BSgenome.Hsapiens.UCSC.hg38, flanking)

writeXStringSet(seqs, "/home/tnagano/projects/def-ccastel/tnagano/TFAMKO_mtDNA-CN/data/fasta/sig_genes_upstream.fa,, format="fasta")