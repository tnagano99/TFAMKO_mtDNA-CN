...
df_sigCpGs <- filter(summaryStats, pvalue < 1e-7)
...
write.csv(sigGO, paste(baseDir, "/results/data/GO_LMM.csv", sep=""))
...
write.csv(sigKEGG, paste(baseDir, "/results/data/KEGG_LMM.csv", sep=""))
...
# colnames(merged)[colnames(merged) == "pval"] <- "P.Value"
